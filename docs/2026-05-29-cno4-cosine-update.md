...
The Census run uses the same CNO4 parsed records and O*NET-enriched Anthropic embeddings as the EPA run. The difference is aggregation:

- CNO4 predictions are aggregated to CNO2 with equal CNO4 weights.
- No INE table 65134 employment weighting is used for Census, because the Census branch's occupation variable is already the 2-digit CNO category.
- Industry-period exposure is then computed by matching those CNO2 predictions to Census `OCU63` and aggregating by Census `ACT89`.

Outputs:

- `data/processed/spanish_census_occupation_exposure.csv`
  - 62 CNO2 rows.
  - Columns include `CNO2`, `OCUP1` (the 2-digit code used for merge compatibility), `CNO1`, `occupation_title`, `cno4_count`, `aggregation_weight_source`, and cosine exposure columns.

- `data/processed/spanish_census_industry_period_exposure.csv`
  - 88 industry-period rows.
  - Includes `industry_name`.
  - Includes only cosine exposure columns, not RF/Ridge/Ensemble.
...
- `data/processed/spanish_census_run_metadata.json`
  - Records source `census`, methods, occupation detail mode, model hash, and output paths.
